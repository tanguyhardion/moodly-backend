...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  try {
    ...
    if (error) {
      ...
      res.status(500).json(createErrorResponse('Failed to fetch entries from database'));
      return;
    }
    ...
    res.status(500).json(createErrorResponse('Internal server error while fetching entries'));
  }
}
