...
export default async function handler(req: VercelRequest, res: VercelResponse) {
  ...
  if (!q || typeof q !== "string") {
    res.status(400).json(createErrorResponse("Query parameter 'q' is required"));
    return;
  }

  const apiKey = process.env.GEOAPIFY_API_KEY;
  if (!apiKey) {
    res.status(500).json(createErrorResponse("Geoapify API key not configured"));
    return;
  }
        ...
        .get(url, (apiRes) => {
          ...
          apiRes.on("end", () => {
            if (apiRes.statusCode && apiRes.statusCode >= 200 && apiRes.statusCode < 300) {
              ...
              reject(new Error(`Geoapify API error: ${apiRes.statusCode} ${body}`));
            }
          });
        })
